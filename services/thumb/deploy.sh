...
# Script to deploy the thumb service to Kubernetes
...
echo "Deploying thumb service to '$TIER' tier..."

# Check if the image exists
if ! docker image inspect thumb:$TAG >/dev/null 2>&1; then
  echo "Error: Docker image 'thumb:$TAG' not found"
...
kubectl apply -f $MANIFEST_DIR/thumb-deployment.yaml
kubectl apply -f $MANIFEST_DIR/thumb-service.yaml

echo "Deployment complete. Checking status..."
kubectl get pods -l app=thumb

echo "To access the thumb service:"
echo "kubectl port-forward svc/thumb 8001:80"
echo "Then you can use curl or a browser to access http://localhost:8001"
